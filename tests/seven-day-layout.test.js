...
test('line: a hidden model-scoped window cannot hold the weekly line up by itself', () => {
  // showModelScopedUsage: false must gate the weekly line's threshold check the
  // same way it gates the usage line's, or the weekly line outlives its row.
  const ctx = baseContext(
    { sevenDayLayout: 'line', usageThreshold: 90, showModelScopedUsage: false },
    { fiveHour: 10, sevenDay: 85, scopedWindows: [{ label: 'Fable', percent: 95, resetAt: null }] },
  );
  assert.equal(renderUsageLine(ctx), null);
  assert.equal(renderWeeklyUsageLine(ctx), null);
});
