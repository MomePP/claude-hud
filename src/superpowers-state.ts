...
interface LedgerInfo {
  /** null when the ledger does not state a total (prose format) — derive from todos. */
  total: number | null;
  completed: number;
  inProgress: number;
  objective: string;
 ...
 */
function findSddBase(cwd: string): string | null {
...
function findLedgerFile(base: string): { file: string; slug: string; mtime: Date } | null {
  let best: { file: string; slug: string; mtime: Date } | null = null;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!best || stat.mtime > best.mtime) best = { file, slug: entry.name, mtime: stat.mtime };
    } catch {
...
function parseLedger(raw: string, slug: string, mtime: Date): LedgerInfo {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (plan) {
      if (!objective) objective = path.basename(plan[1], '.md');
      continue;
    }
    const task = line.match(LEDGER_TASK);
    if (task) {
      seen.add(task[1]);
      if (/^complete\b/i.test(task[2])) done.add(task[1]);
      continue;
    }
    const box = line.match(CHECKBOX);
    if (box) {
      boxTotal += 1;
      if (box[1].toLowerCase() === 'x') boxDone += 1;
      continue;
    }
    if (!objective) {
...
function readLedger(cwd: string): LedgerInfo | null {
  try {
