...
import { getModelName, formatModelName, getProviderLabel, resolveModelName } from '../../stdin.js';
...
function getProjectPath(cwd: string | undefined, pathLevels: RenderContext['config']['pathLevels']): string | null {
  if (!cwd) return null;
  return formatProjectPath(cwd, pathLevels ?? 1) || null;
}

function buildExtras(ctx: RenderContext): FirstLinePart[] {
  const display = ctx.config?.display;
  const colors = ctx.config?.colors;
  const extras: FirstLinePart[] = [];
  const push = (text: string, key: FirstLineSegment | null = null) => extras.push({ key, text });
  ...
  if ((display?.showOrchestration ?? true) && ctx.orchestration?.active && ctx.orchestration.mode) {
    ...
    if (inlineDetail) badge += label(`: ${sanitizeDisplayText(objective)}`, colors);
    if (progress) badge += dim(progress);
    push(badge);
  }
...
function renderPipesProjectLine(ctx: RenderContext): string | null {
  ...
  if (vcs) {
    ...
    if (vcs.conflict) gitInner.push(criticalColor('!conflict', colors));
...
function renderNaturalProjectLine(ctx: RenderContext): string | null {
  ...
  if (vcs) {
    ...
    if (vcs.conflict) gitTokens.push(criticalColor('!conflict', colors));
...
export function renderGitFilesLine(ctx: RenderContext, terminalWidth: number | null = null): string | null {
  ...
  if (!fileListEnabled) return null;
