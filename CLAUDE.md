...
- **`colors.thinking`, `colors.duration`, and `colors.orchestration` stay** as independent overrides. Upstream periodically tries to consolidate them into `colors.label`; refuse.
...
| `display.projectStyle: 'natural'` | `src/render/lines/project.ts` (`renderNaturalProjectLine`) | Starship-style prose layout. It composes its own model segment, so anything upstream adds to the pipes bracket (`formatModelDisplay`) has to be ported by hand — the effort suffix was inert here until 0.12.0. Shares `formatEffortSuffix` (`src/render/model-display.ts`) and `buildInlineAddedDirs` (same file) rather than reimplementing them — keep both shared on sync. Everything else on the project line comes from `buildExtras`, which both styles call; when auditing parity, diff `renderPipesProjectLine` against `renderNaturalProjectLine` for `display?.*` reads. |
| Usage-window divider | `src/render/lines/usage.ts` (`usageWindowSeparator`) | The 5h↔weekly divider follows the merge separator (`naturalSeparator` under `natural`, ` │ ` under `pipes`) rather than the hardcoded ASCII `\|` upstream uses. Upstream tests asserting a literal `\|` between the windows must be adapted to assert separation, not the glyph. The scoped-window and balance joins keep their own `\|`. |
| `colors.sevenDay` | `src/config.ts`, `src/render/lines/usage.ts` (`sevenDayColors`), `src/render/session-line.ts` | Optional (`HudColorValue \| undefined`, no default) per-window colour for the weekly quota — same optional shape as `barFilled`/`barEmpty`/`barEmptyColor`, so upstream pressure to give it a default must be refused. Pins `usage`/`usageWarning`/`critical` for that window only, which is how it leaves the 75/90 ladder. Six weekly render paths route through the helper; the 5-hour window, memory bar and **model-scoped** windows must not (scoped windows carry a `7d` label but are a different quota). |
