...
function takeLast(entries, count) {
...
export function renderAgentsLine(ctx) {
    ...
    const runningAgents = takeLast(agents
        .filter((agent) => agent.status === 'running')
        .filter(unique), MAX_AGENTS_SHOWN);
    const completedSlots = Math.min(MAX_RECENT_COMPLETED, MAX_AGENTS_SHOWN - runningAgents.length);
    const recentCompleted = takeLast(agents
...
function formatAgent(agent, colors, namespaceMode, now) {
    ...
    const typeLabel = truncateString(formatNamespaced(cleanedType, namespaceMode), AGENT_TYPE_MAX_LEN) || 'agent';
...
function sanitizeAgentText(value, maxLen) {
    if (typeof value !== 'string')
        return '';
    return truncateString(sanitizeDisplayText(value).trim(), maxLen);
}
function formatElapsed(agent, now) {
