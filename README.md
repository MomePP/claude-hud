...
| — | **Unified orchestration awareness.** A source-selectable phase/mode badge on the project line: `✦ <phase> c/t` for [superpowers](https://github.com/obra/superpowers) (phase = the transcript's latest `superpowers:<skill>`; task counts from the plan's SDD ledger at `<repo-root>/.superpowers/sdd/<plan>/progress.md`, or todos) or `⚙ <mode> c/t` for OMC (read from `<cwd>/.omc/state/`). `display.orchestrationSource` (`auto`/`superpowers`/`omc`/`off`) picks the ecosystem; `display.showOrchestration` toggles the inline badge and `display.showOrchestrationDetail` an optional detail, placed by `display.orchestrationDetailLayout` (`line` for its own line, `inline` to fold `<mode>: <objective> c/t` into the badge instead). Legacy `showOmcMode`/`showOmcState` migrate automatically |
...
| `projectLineOrder` reorders first-line segments in every layout | Applies to `projectStyle: "pipes"` and compact layout. `natural` style composes prose ("in X on Y"), so permuting its segments would break the grammar — it keeps its fixed order |
...
- **Upstream drift.** Not a live mirror. The fork is periodically **rebased onto the current upstream base** — upstream as the root, fork patches replayed cleanly on top, linear history — rather than merged (which would leave `main` carrying both lineages). Most recently synced to upstream [`6f065f2`](https://github.com/jarrodwatts/claude-hud) (post-0.7.0, landed in fork 0.9.0), adopting jj (Jujutsu) status, `projectLineOrder`, `pathLevels: "full"`, `display.rightAlign`, auth display, `modelSource`, Traditional Chinese, Claude 5 / MiniMax pricing, MCP error reporting, wall-clock hour-cycle options, and the Windows git-runner hardening. See `CLAUDE.md` → "Merging from Upstream" for the procedure.

## Install

> **Latest release: [v0.9.1](https://github.com/MomePP/claude-hud/releases/tag/v0.9.1).** `/plugin install` always pulls the newest version from the marketplace — no version pinning needed.
...
Linux users: on older Claude Code versions, `/tmp` being a separate filesystem (tmpfs) made plugin installation fail with `EXDEV: cross-device link not permitted`. That [Claude Code bug](https://github.com/anthropics/claude-code/issues/14799) is now fixed — update Claude Code first. If you can't update, set `TMPDIR` to a path on the same filesystem as your home directory before installing:
...
```
- **Line 1** — Model, provider label when positively identified (for example `Bedrock`, `Vertex`, `MiniMax`), project path, git branch, and an orchestration phase badge when active (`✦ executing-plans 3/7` for superpowers, `⚙ <mode> 2/5` for OMC)
...
| `projectLineOrder` | string[] | `[]` | Optional leading order of segments *within* the first line. Visibility stays with the `display.show*` flags, and omitted segments keep their existing renderer order. `model` covers provider + model + effort (plus the context bar in compact mode); `project` covers path + added dirs + git as one segment. Example: `["project","model"]` puts the project/git block before the model badge. Applies to `projectStyle: "pipes"` and compact layout; `natural` style composes prose and keeps its fixed order. |
...
| `display.showPromptCache` | boolean | false | Show the wall-clock time the session's prompt cache expires, read from the transcript (`promptCache` element). Inherited from upstream. |
| `display.promptCacheTtlSeconds` | number | 300 | Compatibility fallback used only when the transcript has not reported a 5-minute or 1-hour cache tier. Inherited from upstream. |
...
| `display.hourCycle` | `auto` \| `h11` \| `h12` \| `h23` \| `h24` | `auto` | Hour cycle for wall-clock reset times (`absolute` / `both` / `elapsedAndAbsolute`). `auto` defers to the system locale; `h23` forces 24-hour time (`14:30`) regardless of locale. Inherited from upstream. |
| `display.showClockSeconds` | boolean | false | Include seconds in wall-clock reset times, e.g. `at 14:30:07`. Inherited from upstream. |
...
| `display.showModelScopedUsage` | boolean | true | Show the per-model weekly windows (`model_scoped`, e.g. Fable), whether they arrive on stdin or from the external usage snapshot. Set to `false` to render the usage line as if the payload carried none of them; a hidden window no longer counts toward `usageThreshold`. Inherited from upstream. |
...
| `display.showOrchestrationDetail` | boolean | `false` | Opt-in detail surfacing the active orchestration: `✦`/`◆ <mode>: <objective> (done/total) · N agents`. Off by default. Placement is set by `orchestrationDetailLayout`. |
| `display.orchestrationDetailLayout` | `'line'` \| `'inline'` | `'line'` | Where that detail goes once enabled. `line` renders it as its own line. `inline` instead folds the objective into the project-line badge — `✦ <mode>: <objective> done/total` — and drops the separate line, so the detail shows in one place, never both. The agent count stays on the `line` form only. Requires the badge (`showOrchestration`) and the `expanded` layout; under `compact`, or with the badge off, or when the ledger yields no objective, it falls back to the separate line rather than dropping the detail. |
...
> **Orchestration sources.** For **superpowers**, the phase is the most-recent `superpowers:<skill>` invocation in the transcript (e.g. `executing-plans`); task progress is enriched by the plan's SDD ledger — superpowers ≥6.2 scopes one workspace per plan at `<repo-root>/.superpowers/sdd/<plan-basename>/progress.md`, and the newest ledger wins when several plans coexist. Completions come from its `Task <N>: complete` lines and the total from the todo list; the pre-6.2 flat `.superpowers/sdd/progress.md` is deliberately ignored, since superpowers leaves stale ledgers in place as another plan's progress. A checkbox-style ledger still parses when it holds no task lines. Note that superpowers only deletes a plan workspace once its final review is clean, so an abandoned plan leaves its ledger behind and keeps the badge alive — remove the stale `.superpowers/sdd/<plan>/` directory to clear it. For **OMC** (oh-my-claudecode), state is read from `<cwd>/.omc/state/`. The legacy `display.showOmcMode` / `display.showOmcState` keys still work — they migrate to `showOrchestration` / `showOrchestrationDetail`.
...
Official MiniMax Anthropic-compatible endpoints get a `MiniMax` provider label. MiniMax M2.7 uses its published token and cache prices for local estimates; M3 pricing depends on each request's context tier, which cumulative session tokens cannot safely infer, so ClaudeHUD does not guess an M3 estimate.
...
ClaudeHUD prefers the official statusline stdin payload for rate-limit windows. If `display.externalUsagePath` points to a fresh local sidecar snapshot, ClaudeHUD can append its `balance_label` alongside stdin windows, and fill in `model_scoped` windows when stdin lacks them. If stdin `rate_limits` are missing, the same snapshot can provide fallback usage windows.
